...
set REDIRECT_URI "https://request.fschen.de/mviwwff"
...
"
        ...
        if echo $CALL_RESPONSE | python3 -c "import sys,json; d=json.load(sys.stdin); exit(0 if 'result' in d else 1)" 2>/dev/null
            pass "tools/call erfolgreich!"
            echo $CALL_RESPONSE | python3 -c "import sys,json; print(json.dumps(json.load(sys.stdin),indent=2)[:500])"
        else
