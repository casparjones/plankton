...
Die Kommunikation zwischen den Agenten erfolgt ausschließlich über Dateien im `/vibe/`-Verzeichnis.
...
**Verantwortung:**
- Liest beim Start immer zuerst diese Datei (`/vibe/readme.md`) vollständig durch
- Analysiert die gesamte aktuelle Codebasis auf fehlende Features, Bugs, technische Schulden
- Liest alle Dateien in `/vibe/ideas/` – erstellt daraus Epics und verschiebt die Idee nach `/vibe/done/ideas/`
- Erstellt Epics in `/vibe/epics/<epic-id>.md` (Format siehe unten)
- Bricht Epics in konkrete Tasks auf und legt diese in `/vibe/tasks/<task-id>.md` ab
...
- Verschiebt abgeschlossene Tasks: `/vibe/tasks/<task-id>.md` → `/vibe/done/tasks/<task-id>.md`
- Prüft danach das zugehörige Epic – welche Tasks fehlen noch? → nächste Task-Datei erstellen
- Wenn alle Tasks eines Epics erledigt sind: Epic nach `/vibe/done/epics/<epic-id>.md` verschieben

**Manager startet IMMER mit diesen Schritten:**
1. `/vibe/readme.md` lesen
2. Codebasis analysieren (`src/main.rs`, `static/main.js`, `static/styles.css`, `Cargo.toml`, `package.json`)
3. `/vibe/ideas/` auf neue Ideen prüfen
4. Offene Epics in `/vibe/epics/` prüfen – gibt es Tasks die noch nicht erstellt wurden?
5. Offene Tasks in `/vibe/tasks/` prüfen – gibt es Tasks ohne Assignee?
...
### Epic-Datei `/vibe/epics/<epic-id>.md`
...
### Task-Datei `/vibe/tasks/<task-id>.md`
...
1. **Immer** `cargo build` nach Backend-Änderungen ausführen
2. **Immer** Task-Dateien aktuell halten (Status, Logs, Notes)
3. **Keine** Breaking Changes ohne explizite Erwähnung im Task
4. **Deutsche** Code-Kommentare, englische Variablen-/Funktionsnamen
5. Bei Unklarheiten: in dem Plankton Task dokumentieren, nicht raten
6. Tester prüft **immer** ob `cargo build` ohne Fehler durchläuft
7. Kein Agent löscht keine Task – nur verschieben in DONE
---
